...
import {transferSecondList,twoTransferCheck} from '@/api/rollInOrOut'
import SearchList from '@/components/SearchList'
import { isEmpty, dataType } from '@/utils/auth'

export default {
    name:'twoRollOutAudit',
    data() {
        return {
          loading:false,
          tableData:[],
    ...
    methods:{
      ...
      deleteData(id,type){
        twoTransferCheck({id},type).then(res=>{
          if(res.statusCode === 0){
            this.getArticleByLocaleList()
          }
        })
      ...
      getArticleByLocaleList(data){
        ...
        transferSecondList(parmas).then(res=>{
