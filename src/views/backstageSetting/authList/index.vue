...
import {getUserAuthList,exportPermissionList,delPermissionList} from '@/api/backstageSetting'
...
export default {
    ...
    methods:{
      ...
      getDetail(id){
        this.$router.push({
          path:'/backstage-setting/auth-operation',
          query:{
            id
          }
        })
      ...
      exportDate(data){
        ...
        exportPermissionList({id:0}).then(res=>{
          download(res,'权限列表')
      ...
      add(){
        this.$router.push('/backstage-setting/auth-operation')
      },
      deleteData(id){
      ...
      getArticleByLocaleList(data){
        ...
        getUserAuthList(parmas).then(res=>{
